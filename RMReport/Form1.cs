...
namespace RMReport
{
    public partial class Form1 : Form
    {
        ...
        public Form1(string rmf, string dbFile, Hashtable dataDict, int action)
        {
            InitializeComponent();
        ...
        ///</summary>
        public void My_OpenSql(string StrSql, System.Data.OleDb.OleDbConnection OleDbCon, DataTable T_Data)
        {
            System.Data.OleDb.OleDbDataAdapter DbCom;
            DbCom = new System.Data.OleDb.OleDbDataAdapter();
            if (OleDbCon.State != ConnectionState.Open)
            {
                OleDbCon.Open();
            }
            DbCom.SelectCommand = new System.Data.OleDb.OleDbCommand(StrSql, OleDbCon);
            T_Data.Clear();
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            this.OleDbCon.ConnectionString = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + mainPath + this.dbFile + "; Persist Security Info=False";
        ...
        private void PrintReport(int IntKind)
        {
            ...
            MClearAll(); //清除所有数据源及变量
                         //oType.InvokeMember("MAddSn( "", "" ); //加入注册码  参数1(StrName):注册名,参数2(StrSn):注册码
                         //1、变量的设置方法
                         //Fr.MAddV(StrVName as String ,Vvalue),参数说明:：1、参数1(StrVName)变量名称;2、参数2(Vvalue)变量值(可以是字符串,数值,日期,是否等类型)
            MAddV("V1", "中国");
            MAddV("V2", 1);
            MAddV("V3", 0.87);
            MAddV("V4", DateTime.Now);
            MAddV("V5", false);
            MAddV("V6", "sadfasd");

            MAddV("MyV", DateTime.Now);
            ...
            foreach (DictionaryEntry d in this.dataDict)
            {
                ...
                this.My_OpenSql(StrSql, OleDbCon, DtTemp);
